  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-black font-bold text-xl">Attendance Management</h2>
          <p className="text-gray-500 text-sm mt-0.5">Track and manage student attendance</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {stats.map((s) => (
          <motion.div key={s.label} whileHover={{ y: -2 }}
            className="rounded-2xl bg-white border border-gray-200 border border-gray-100 p-4 flex items-center gap-3 cursor-pointer"
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs">{s.display}</p>
            </div>
          </motion.div>
        ))}
      </div>
        ...
        <div className="flex rounded-xl overflow-hidden border border-gray-100">
          {["All", "Safe", "Warning", "Defaulter"].map((f) => (
            <button key={f} onClick={() => setFilter(f as any)}
              className={cn("px-3 py-2 text-sm font-medium transition-colors", filter === f ? "bg-amber-500 text-black" : "bg-gray-50 text-gray-500 hover:bg-gray-100")}>
              {f}
            </button>
          ))}
        </div>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <motion.tr key={`${a.studentId}|${a.subjectId}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.03 }}
                  ...
                  <td className="px-4 py-3 text-black text-sm font-medium whitespace-nowrap">{a.studentName}</td>
                  <td className="px-4 py-3 text-gray-700 text-sm whitespace-nowrap">{a.department}</td>
                  <td className="px-4 py-3 text-gray-500 text-sm whitespace-nowrap">{a.subject}</td>
                  <td className="px-4 py-3 text-gray-700 text-sm text-center">{a.semester}</td>
                  <td className="px-4 py-3 text-emerald-400 text-sm font-semibold">{a.present}</td>
                  <td className="px-4 py-3 text-gray-700 text-sm">{a.total}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-700 font-semibold">{a.percentage}%</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
