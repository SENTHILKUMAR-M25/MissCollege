  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-black font-bold text-xl">Result Management</h2>
          <p className="text-gray-500 text-sm mt-0.5">Publish and analyze semester results</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {statCards.map((s) => (
          <div key={s.label} className="bg-white border border-gray-200 border border-gray-100 rounded-2xl p-5 relative overflow-hidden">
            ...
            <p className="text-black text-3xl font-black relative z-10">{s.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 rounded-2xl bg-white border border-gray-200 border border-gray-100 p-5">
          <h3 className="text-black font-bold mb-4 flex items-center gap-2"><Award size={18} className="text-amber-400" /> Top Rank Holders</h3>
          {topRankers.length === 0 ? (
            <p className="text-gray-400 text-sm text-center py-8">No results data available.</p>
          ) : (
            <div className="space-y-3">
              {topRankers.map((s) => {
                const colors = ["from-[#2F2FE4] to-[#4F6FE4]", "from-slate-300 to-slate-400", "from-amber-700 to-orange-800", "from-[#2F2FE4] to-[#4F6FE4]", "from-blue-400 to-cyan-500"]
                return (
                  <div key={s.reg} className="flex items-center gap-4 p-3 bg-gray-100 rounded-xl border border-gray-100">
                    <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center text-black font-black text-lg bg-gradient-to-br shadow-lg", colors[s.rank - 1] ?? "from-slate-500 to-slate-600")}>
                      #{s.rank}
                    </div>
                    <div className="flex-1">
                      <p className="text-black font-bold text-sm">{s.name}</p>
                      <p className="text-gray-500 text-xs">{s.reg} • {s.dept}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-emerald-400 font-black text-xl">{s.cgpa}</p>
                      <p className="text-gray-400 text-[10px] uppercase tracking-wider">CGPA</p>
                    </div>
                  </div>
                )
              })}
        ...
        <div className="rounded-2xl bg-white border border-gray-200 border border-gray-100 p-5">
          <h3 className="text-black font-bold mb-4 text-sm">Pass Percentage by Semester</h3>
          <div className="space-y-4 mt-6">
            {passRateBySem.map((s) => (
              <div key={s.semester}>
                <div className="flex justify-between text-xs mb-1.5">
                  <span className="text-gray-700 font-medium">{s.semester}</span>
      ...
      {filtered.length > 0 && (
        <div className="rounded-2xl bg-white border border-gray-200 border border-gray-100 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <h3 className="text-black font-semibold text-sm">Result Records ({filtered.length})</h3>
          </div>
              ...
              <tbody>
                {filtered.slice(0, 50).map((r, idx) => (
                  <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.02 }}
                    ...
                    <td className="px-4 py-3 text-black text-sm font-medium">{r.studentName}</td>
                    <td className="px-4 py-3 text-gray-700 text-sm">{r.department}</td>
                    <td className="px-4 py-3 text-gray-500 text-sm">{r.subjectCode} - {r.subjectName}</td>
                    <td className="px-4 py-3 text-gray-700 text-sm text-center">{r.semester}</td>
                    <td className="px-4 py-3 text-gray-700 text-sm font-semibold">{r.mark}</td>
                    <td className="px-4 py-3">
                      <span className={cn("px-2.5 py-1 rounded-lg text-xs font-bold", gradeColor[r.grade] ?? "text-gray-500 bg-slate-500/10")}>
                        {r.grade}
                      </span>
                    </td>
                  </motion.tr>
