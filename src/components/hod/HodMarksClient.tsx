  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Total Records", value: filtered.length, color: "text-black" },
          { label: "Subjects", value: new Set(filtered.map(m => m.subject.id)).size, color: "text-[#2F2FE4]" },
          { label: "Students", value: new Set(filtered.map(m => m.student.id)).size, color: "text-emerald-400" },
          { label: "Avg Mark", value: filtered.length > 0 ? Math.round(filtered.reduce((a, b) => a + b.mark, 0) / filtered.length) : 0, color: "text-[#2F2FE4]" },
        ].map((stat) => (
          ...
          return (
            <div key={key} className="rounded-2xl bg-gray-50 border border-gray-100 p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="text-black text-sm font-bold">{subject?.code} - {subject?.name}</p>
                  <p className="text-gray-500 text-xs">Sem {subject?.semester} • {examType}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-gray-500 text-xs">Avg: <span className="text-black font-semibold">{avg}%</span></span>
                  <span className={`px-2 py-1 rounded-lg text-[10px] font-bold border ${EXAM_COLORS[examType] || "bg-gray-100 text-gray-500 border-gray-300"}`}>{examType}</span>
                </div>
              </div>
            </div>
          )
