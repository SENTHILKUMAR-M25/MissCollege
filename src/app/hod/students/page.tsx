    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-black text-2xl font-bold">Student Management</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {Object.entries(bySemester).map(([sem, list]) => (
          <div key={sem} className="rounded-2xl bg-white border border-gray-200 p-4">
            <p className="text-gray-400 text-[10px] font-semibold uppercase tracking-wider">Semester {sem}</p>
            <p className="text-2xl font-bold text-black mt-1">{list.length}</p>
      ...
      {Object.entries(bySemester).map(([sem, list]) => (
        <div key={sem} className="rounded-2xl bg-white border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <div>
              <h3 className="text-black font-bold">Semester {sem}</h3>
              <p className="text-gray-400 text-xs mt-0.5">{list.length} students • {me.department.code}</p>
            </div>
                ...
                {list.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="border-b border-gray-100 hover:bg-gray-100 transition-colors">
                      <td className="px-4 py-3 text-[#2F2FE4] text-sm font-mono font-semibold">{s.registerNumber}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-[#2F2FE4]/10 flex items-center justify-center text-[#2F2FE4] font-bold text-xs">
                            {s.user.name?.charAt(0) || "S"}
                          </div>
                          <span className="text-black text-sm">{s.user.name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-gray-700 text-sm">{s.course.code}</td>
                      <td className="px-4 py-3 text-gray-700 text-sm">{s.section || "–"}</td>
